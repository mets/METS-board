...
METS ist ein Kodierungsstandard für beschreibende, administrative/technische und strukturelle Metadaten von komplexen digitalen Objekten unterschiedlichster Art, bspw. Text-, Bild- oder Multimedia-Objekte. Ursprünglich eine Initiative der [Digital Library Federation](http://www.diglib.org/) ist METS ein XML-Dokumentformat für die Kodierung von Metadaten sowohl für die Verwaltung von digtialen Objekten innerhalb eines digitalen Magazins als auch für den Austausch solcher Objekte zwischen den digitalen Magazinen und den Benutzern/Benutzerinnen. Abhängig vom jeweiligen Anwendungsfall können METS-Dokumente im Kontext des [Open Archival Information System (OAIS)-Referenzmodells.](http://ssdoo.gsfc.nasa.gov/nost/isoas/ref_model.html) \[für die langfristige Erhaltung digitaler Aufzeichnungen\] entweder als Liefereinheit (Submission Information Package - SIP), Archivierungseinheit (Archival Information Package - AIP), oder Bereitstellungseinheit (Dissemination Information Package - DIP) verwendet werden.

METS bietet ein kohärentes, integriertes Modell für die komplexen Metadaten, die für die Pflege von einer Sammlung digitaler Objekte benötigt werden. Diese Metadaten sind sowohl ausführlicher als auch unterschiedlich im Vergleich zu den Informationen über Sammlungen von gedruckten Werken oder anderen physischen Objekten. Ein gedrucktes Buch bspw. wird weder in einzelne Blätter zerfallen wenn keine Strukturangaben über die innere Ordnung des Buches erfasst werden, noch werden Forscher das Buch schlechter nutzen können wenn Angaben über die Herstellungsart fehlen. Gleiches gilt jedoch nicht für eine digitale Version desselben Buches. Ohne Metadaten zur Struktur sind die Seitenabbildungen oder die Textdateien, aus denen es besteht, so gut wie wertlos. Und ohne technische Metadaten über den Digitalisierungsprozess können Leser nicht sicher sein, wie genau die digitale Version die ursprüngliche Vorlage wiedergibt. Eine Organisation muss auch notwendige technische Metadaten für interne Verwaltungszwecke verfügbar haben, um bspw. die Daten regelmässig überprüfen und migrieren zu können und damit langfristige Zugänglichkeit der wertvollen Quellen zu garantieren.
